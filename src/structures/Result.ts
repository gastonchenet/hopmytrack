...
export default class Result {
  ...
  public static fromSearchData(data: SearchData): Result {
    ...
    if (data.email) result.addEmail(data.email.toLowerCase(), Prob.SURE);
  ...
  public emails: ProbValue<string>[] = [];
  ...
  public addUsername(username: string, prob: number): void {
    ...
    if (foundUsername) {
      prob = (Math.max(prob, foundUsername.prob) + Prob.SURE) / 2;
      this.prob = prob;
    }
  ...
  public addFirstName(firstName: string, prob: number): void {
    ...
    if (existingFirstName?.value === firstName) {
      prob = (Math.max(prob, existingFirstName.prob) + Prob.SURE) / 2;
      this.prob = prob;
    }
  ...
  public addLastName(lastName: string, prob: number): void {
    ...
    if (existingLastName?.value === lastName) {
      prob = (Math.max(prob, existingLastName.prob) + Prob.SURE) / 2;
      this.prob = prob;
    }
  ...
  public addEmail(email: string, prob: number): void {
    const existingEmail = this.findEmail(email);
    if (existingEmail?.prob === Prob.SURE) return;

    if (existingEmail) {
      existingEmail.prob =
        (Math.max(existingEmail.prob, prob * this.prob) + Prob.SURE) / 2;
    } else {
      this.emails.push({ value: email, prob: prob * this.prob });
    }
